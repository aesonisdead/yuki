	...
	write = async (data) => {
		...
		try {
			...
			if (Object.keys(this.data).length > 0) {
				fs.writeFileSync(this.file, safeStringify(this.data));
			}
		} catch (e) {
...
const dataBase = (source) => {
	if (/^mongodb(\+srv)?:\/\//i.test(source)) return new MongoDB(source);
...
const cmdAddHit = (hit, feature) => {
	if (hit && !hit[feature]) hit[feature] = 0;
	if (hit) hit[feature]++;
}

const addExpired = ({ id, expired, ...options }, _dir) => {
	const _cek = _dir.find(a => a.id == id);
	if (_cek) _cek.expired = _cek.expired + toMs(expired);
	else _dir.push({ id, expired: Date.now() + toMs(expired), ...options });
};
	...
	setInterval(() => {
		for (let i = _dir.length - 1; i >= 0; i--) {
			if (Date.now() >= _dir[i].expired) {
				if (conn) conn.groupLeave(_dir[i].id).catch(e => {});
